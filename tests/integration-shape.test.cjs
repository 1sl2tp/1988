...
assert.match(refresh,/function exactSearchVideoMeta\(data:any,id:string\)/);
assert.match(refresh,/async function youtubeSearchVideoMetadata/);
assert.match(refresh,/youtubeSearchVideoMetadata\(supabaseUrl,serviceKey,candidate\.id\)/);
assert.match(refresh,/data\?\.name\|\|data\?\.title/);
assert.match(refresh,/const sourceMetaUpdates=\[\.\.\.channelMeta\.values\(\)\]/);
assert.match(refresh,/yt1988_source_state\?profile_key=eq\./);
...
assert.match(app,/const legacyHash=snapshotRowsHashV330\(items,sourceSig\)/);
assert.match(app,/serverHash:expectedHash/);
assert.match(app,/knownLocalHashes=new Set/);
assert.match(refresh,/Number\(r\?\._shortCheckedAt\)>0/);
assert.doesNotMatch(refresh,/reason:["']waiting_non_live_verification["']/);
...
const swVersion=html.match(/sw\.js\?v=([A-Za-z0-9._-]+)/)?.[1]||"";
assert.ok(swVersion,"index.html must version sw.js");
assert.ok(sw.includes("1988-main-watch-browse-v"+swVersion),"sw.js cache version must match index.html");
