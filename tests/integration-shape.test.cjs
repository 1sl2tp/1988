...
assert.doesNotMatch(refresh,/if\(strongFreshLiveSignal\(row\)\)return/);
...
assert.match(refresh,/const LIVE_PIPELINE_VERSION=["']live-v18["']/);
...
assert.match(refresh,/const externalExcludedSourceIds=new Set<string>\(allBlockedLiveSourceIds\)/);
assert.match(refresh,/for\(const id of allSelectedLiveSourceIds\)externalExcludedSourceIds\.add\(id\)/);
assert.match(refresh,/const globalCandidates=await discoverGlobalLiveCandidates/);
assert.match(refresh,/const verifiedExternalRows=await verifyLiveRows/);
assert.match(refresh,/const selectedDiscoveryBatches=await mapLimit\(selectedLiveSources/);
assert.match(refresh,/const verifiedSelectedRows=await verifyLiveRows/);
assert.match(refresh,/verifiedLiveRowsCache=dedupeRows\(\[/);
assert.doesNotMatch(refresh,/const selectedDiscoveryPromise=/);
assert.doesNotMatch(refresh,/const globalDiscoveryPromise=/);
